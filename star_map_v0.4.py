...
#		print(self.kp_catalog,self.kp_img)
		if self.kp_catalog.shape[0]>5:
			mtx,inliers=cv2.estimateAffinePartial2D(self.kp_catalog_original,self.kp_img)
...
#			mtx,inliers=cv2.estimateAffine2D(self.kp_catalog_original,self.kp_img)
			...
			mtx=np.array([[1,0,0],[0,1,0]])
			inliers=np.array([[0]])
#		print(mtx.shape)
#		print(self.mtx_old.shape)
		insert_m=[0,0,1]
		mtx=np.insert(mtx,2,insert_m,axis=0)
		self.mtx_old=np.insert(self.mtx_old,2,insert_m,axis=0)
		print(mtx)
		print(self.mtx_old)
		mtx=np.dot(mtx,self.mtx_old)
...
#		stars_catalog_calc=np.insert(self.stars_catalog_original, 2, 1, axis=1)	#add
		stars_catalog_re=[]
		for s_point in stars_catalog_calc:
			s_point=np.dot(mtx,s_point)
